...
//// --- Begin LibAFL code ---
    ...
    if (gen == gen_sub_CC || /*gen == gen_add_CC ||*/ gen == gen_rsb_CC) {
      ...
      libafl_gen_cmp(&s->base, tcg_env, s->pc_curr, (s->pc_curr-s->pc_save), tmp1_64, tmp2_64, MO_32);
#else
      libafl_gen_cmp(&s->base, tcg_env, s->pc_curr, (s->pc_curr-s->pc_save), tmp1, tmp2, MO_32);
#endif
    }
...
//// --- Begin LibAFL code ---
    ...
    if (gen == gen_sub_CC || /*gen == gen_add_CC ||*/ gen == gen_rsb_CC) {
      ...
      libafl_gen_cmp(&s->base, tcg_env, s->pc_curr, (s->pc_curr-s->pc_save), tmp1_64, tmp2_64, MO_32);
#else
      libafl_gen_cmp(&s->base, tcg_env, s->pc_curr, (s->pc_curr-s->pc_save), tmp1, tcg_constant_i32(imm), MO_32);
#endif
    }
