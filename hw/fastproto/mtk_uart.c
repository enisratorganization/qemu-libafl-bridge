...
OBJECT_DECLARE_SIMPLE_TYPE(mtk_uartState, mtk_uart)

#define NUM_GPIO_OUT 1
#define NUM_GPIO_IN 1

struct mtk_uartState {
...
/**
 * Handle input pins. Automatically sets the bits to "level_in" of mtk_uartState
 */
static void mtk_uart_gpio_set(void *opaque, int line, int level)
{
    mtk_uartState *s = (mtk_uartState *) opaque;
...
/**
 * VMState automatically saves all direct struct members of mtk_uartState below _vmstate_saved_offset.
 * If you need to save custom objects, just add them before VMSTATE_END_OF_LIST()
 */
static const VMStateDescription vmstate_mtk_uart = {
    .name = "mtk_uart",
    .version_id = 1,
    .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        {
        .name = "buf", .version_id = 1,.field_exists = 0,.size = sizeof(mtk_uartState)-offsetof(mtk_uartState, _vmstate_saved_offset), 
        .info = &vmstate_info_buffer,.flags=VMS_BUFFER,.offset = offsetof(mtk_uartState, _vmstate_saved_offset),  
        },
        VMSTATE_END_OF_LIST()
    }
};

static void mtk_uart_reset(DeviceState *dev)
{
    mtk_uartState *s = (mtk_uartState *) dev;
}

static void mtk_uart_init(Object *obj)
{
    mtk_uartState *s = (mtk_uartState *) obj;
    DeviceState *dev = DEVICE(obj);
    SysBusDevice *sbd = SYS_BUS_DEVICE(obj);

    memory_region_init_io(&s->mmio1, obj, &mtk_uart_ops, s, "mtk_uart_mmio1", 0x1000);
    sysbus_init_mmio(sbd, &s->mmio1);

    for (int i = 0; i < NUM_GPIO_OUT; i++) { sysbus_init_irq(sbd, &s->out[i]); }
    qdev_init_gpio_in_named(dev, mtk_uart_gpio_set, "in", NUM_GPIO_IN);
}

static void mtk_uart_realize(DeviceState *dev, Error **errp)
{
    mtk_uartState *s = (mtk_uartState *) dev;
...
static void mtk_uart_class_init(ObjectClass *klass, void *data)
{
    ...
    dc->vmsd = &vmstate_mtk_uart;
    dc->realize = &mtk_uart_realize;
    dc->legacy_reset = &mtk_uart_reset;
    device_class_set_props(dc, mtk_uart_properties);
}

static const TypeInfo mtk_uart_info = {
    .name          = TYPE_mtk_uart,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(mtk_uartState),
