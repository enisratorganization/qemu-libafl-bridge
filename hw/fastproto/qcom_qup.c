/**
 * Skeleton of a SysBusDevice that compiles.
 * It allows for convenient and fast prototyping using Copy&Paste, Find&Replace + Coding LLMs
 */

#include "qemu/osdep.h"
#include "qemu/log.h"
#include "qemu/module.h"
#include "qemu/timer.h"
#include "qapi/error.h"
#include "hw/sysbus.h"
#include "migration/vmstate.h"
#include "hw/irq.h"
#include "qom/object.h"
#include "hw/qdev-properties.h"
#include "hw/qdev-properties-system.h"
#include "chardev/char-fe.h"

#define TYPE_qcom_qup "qcom_qup"
OBJECT_DECLARE_SIMPLE_TYPE(qcom_qupState, qcom_qup)

#define NUM_GPIO_OUT 1
#define NUM_GPIO_IN 1

struct qcom_qupState {
    SysBusDevice parent_obj;
    /* MMIO Regions */
    MemoryRegion mmio1;
    MemoryRegion mmio2;
    MemoryRegion mmio3;
    /* GPIO out (irq) */
    qemu_irq out[NUM_GPIO_OUT];
    /* GPIO in (this is a bitfield of input PIN states)*/
    uint64_t level_in;
    /* Properties */
    char *prop_char;
    uint64_t prop_uint64;
    bool prop_bool;

    CharBackend prop_chr;

    /* Put your NOT SAVED members here */

    void* _vmstate_saved_offset;
    /* members below this point are SAVED in the vmstate */
    int nextlen;
};

static const Property qcom_qup_properties[] = {
    DEFINE_PROP_CHR("prop_chr", qcom_qupState, prop_chr),
    DEFINE_PROP_STRING("prop_char", qcom_qupState, prop_char),
    DEFINE_PROP_UINT64("prop_uint64", qcom_qupState, prop_uint64, 0),
    DEFINE_PROP_BOOL("prop_bool", qcom_qupState, prop_bool, 0),
    
};

/**
	regs = {0xe24: 1<<16 | 1<<24, #SE_HW_PARAM_0
			}
	if addr in regs:
		return regs[addr]
	return {0x0: 0x0}.get(addr, 0)*/
static uint64_t qcom_qup_mmio1_read (void *opaque, hwaddr addr, unsigned size) {
    qcom_qupState *s = (qcom_qupState *) opaque;
    uint64_t ret = 0;

    switch (addr) {
    case 0:
        ret = 0;
        break;
    case 0xe24:
        ret = 1 << 16 | 1 << 24; // SE_HW_PARAM_0
        break;

    }
    qemu_log_mask(LOG_TRACE, "%s: off %"HWADDR_PRIx" sz %u val %"PRIx64"\n", __func__, addr, size, ret);
    return ret;
}
static void qcom_qup_mmio1_write (void *opaque, hwaddr addr, uint64_t value, unsigned size) {
    qcom_qupState *s = (qcom_qupState *) opaque;
    qemu_log_mask(LOG_TRACE, "%s: off %"HWADDR_PRIx" sz %u val %"PRIx64"\n", __func__, addr, size, value);

    switch (addr) {
    case 0x270:
        s->nextlen = value;
        break;
    case 0x700:
        qemu_chr_fe_write_all(&s->prop_chr, &value, s->nextlen);
        break;
    default:
        qemu_log_mask(LOG_GUEST_ERROR, "%s: Bad offset 0x%" HWADDR_PRIx "\n", __func__, addr);
    }
}
static const MemoryRegionOps qcom_qup_ops = {
    .read = qcom_qup_mmio1_read,
    .write = qcom_qup_mmio1_write,
    .endianness = DEVICE_NATIVE_ENDIAN,
    .valid = {
        .min_access_size = 4,
        .max_access_size = 4,
    },
};

/**
 * Handle input pins. Automatically sets the bits to "level_in" of qcom_qupState
 */
static void qcom_qup_gpio_set(void *opaque, int line, int level)
{
    qcom_qupState *s = (qcom_qupState *) opaque;
    assert(line >= 0 && line < NUM_GPIO_IN);

    if (level)
        s->level_in |= 1 << line;
    else
        s->level_in &= ~(1 << line);
}

/**
 * VMState automatically saves all direct struct members of qcom_qupState below _vmstate_saved_offset.
 * If you need to save custom objects, just add them before VMSTATE_END_OF_LIST()
 */
static const VMStateDescription vmstate_qcom_qup = {
    .name = "qcom_qup",
    .version_id = 1,
    .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        {
        .name = "buf", .version_id = 1,.field_exists = 0,.size = sizeof(qcom_qupState)-offsetof(qcom_qupState, _vmstate_saved_offset), 
        .info = &vmstate_info_buffer,.flags=VMS_BUFFER,.offset = offsetof(qcom_qupState, _vmstate_saved_offset),  
        },
        VMSTATE_END_OF_LIST()
    }
};

static void qcom_qup_reset(DeviceState *dev)
{
    qcom_qupState *s = (qcom_qupState *) dev;
}

static void qcom_qup_init(Object *obj)
{
    qcom_qupState *s = (qcom_qupState *) obj;
    DeviceState *dev = DEVICE(obj);
    SysBusDevice *sbd = SYS_BUS_DEVICE(obj);

    memory_region_init_io(&s->mmio1, obj, &qcom_qup_ops, s, "qcom_qup_mmio1", 0x1000);
    sysbus_init_mmio(sbd, &s->mmio1);

    for (int i = 0; i < NUM_GPIO_OUT; i++) { sysbus_init_irq(sbd, &s->out[i]); }
    qdev_init_gpio_in_named(dev, qcom_qup_gpio_set, "in", NUM_GPIO_IN);
}

static void qcom_qup_realize(DeviceState *dev, Error **errp)
{
    qcom_qupState *s = (qcom_qupState *) dev;
    //qemu_chr_fe_init(&s->prop_chr, s->prop_chr.chr, &error_abort);
}

static void qcom_qup_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->vmsd = &vmstate_qcom_qup;
    dc->realize = &qcom_qup_realize;
    dc->legacy_reset = &qcom_qup_reset;
    device_class_set_props(dc, qcom_qup_properties);
}

static const TypeInfo qcom_qup_info = {
    .name          = TYPE_qcom_qup,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(qcom_qupState),
    .instance_init = qcom_qup_init,
    .class_init    = qcom_qup_class_init,
};

static void qcom_qup_register_types(void)
{
    type_register_static(&qcom_qup_info);
}

type_init(qcom_qup_register_types)
